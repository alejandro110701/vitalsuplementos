...
describe("missingTokenResult", () => {
  it("tells the client how to authenticate and does not leak a token", () => {
    ...
    expect(result.content[0].text).toContain("preferences/mcp");
    expect(result.content[0].text).not.toMatch(/Bearer [A-Za-z0-9._-]+/);
  });
});

describe("wpcomRpc", () => {
  it("opens a session then POSTs the method as JSON-RPC 2.0", async () => {
    const fetchMock = vi.fn(async (input: string, init?: RequestInit) => {
      const body = JSON.parse(String(init?.body));
      if (body.method === "initialize") {
        return new Response(JSON.stringify({ jsonrpc: "2.0", id: 1, result: { serverInfo: { name: "WordPress.com MCP Server" } } }), {
          status: 200,
    ...
    expect(JSON.parse(String(firstInit.body))).toMatchObject({ jsonrpc: "2.0", method: "initialize" });
    expect((secondInit.headers as Record<string, string>)["Mcp-Session-Id"]).toBe("sess-1");
